...
static int JPEGParseInfo(JPEGIMAGE *pPage, int bExtractThumb)
{
    ...
    // NOTE (ESP-IDF 6 compat): intptr_t instead of int64_t; only the low
    // 4 address bits are used below, identical on 32-bit targets.
...
static int JPEGDecodeMCU_P(JPEGIMAGE *pJPEG, int iMCU, int *iDCPredictor)
{
    ...
    // NOTE (ESP-IDF 6 compat): spelled as negation; left-shifting a
    // negative value is undefined behavior (was ((-1) << bits), identical
    // in two's complement).
    iNegative = -iPositive; // negative bit position being coded
