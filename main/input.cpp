...
{
  static int64_t pressStart = 0;
  static int64_t lastMoveTime = 0;
  ...
  if (t.isPressed())
  {
    if (potentialLongPress && !isMagnifierActive &&
        (now_ms() - pressStart > LONG_PRESS_MS))
    {
    ...
    if (isMagnifierActive)
    {
      ...
      else if (!qualityApplied && (now_ms() - lastMoveTime > 300))
      {
  ...
  if (t.wasReleased())
  {
    ...
    if (!wasMagnifying)
    {
      ...
      if (now_ms() - pressStart < LONG_PRESS_MS)
      {
        if (t.x >= LEFT_ZONE_W)
        {
...
void handleBookConfigTouch(const m5::touch_detail_t &t)
{
      ...
      tapY <= btnY2 + btnH)
  {
    ...
    std::string folder = path_basename(currentMangaPath);
