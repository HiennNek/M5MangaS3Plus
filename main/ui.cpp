...
static const char *TAG = "manga_ui";

// Fast full-page JPEG path (bitbank2 JPEGDEC, same decoder as the Arduino
// build - includes ESP32-S3 SIMD color conversion). Thumbnails and the
// shutdown splash keep using M5GFX's built-in drawJpg via jpeg_helper.
static JPEGDEC jpeg;
...
static int drawMCU(JPEGDRAW *pDraw)
{
  ...
  if (ctx->maxWidth > 0 && cx + cw > ctx->maxWidth)
    cw = ctx->maxWidth - cx;
  if (ctx->maxHeight > 0 && cy + ch > ctx->maxHeight)
    ch = ctx->maxHeight - cy;
  if (cw <= 0 || ch <= 0)
    return 1;
  ...
  if (outX >= sprWidth || outY >= sprHeight)
    return 1;
  if (outX < 0)
  {
    cw += outX;
  ...
  for (int y = 0; y < ch; y++)
  {
    memcpy(&sprBuffer[(outY + y) * sprWidth + outX],
           &pixels[y * pDraw->iWidth], cw * 2);
  }

  return 1;
}

static bool drawJpgWithJpegDec(LGFX_Sprite &spr, uint8_t *buf, size_t size,
...
// Reused PSRAM staging buffer for page JPEGs (avoids malloc/free churn on
// every page turn).
...
// Read a page JPEG from VFS into the staging buffer. Returns file size, or 0
// on failure.
static size_t loadPageFile(const std::string &path)
{
  ...
  ensureJpgBuffer((size_t)len);
  size_t got = 0;
  if (jpgBuffer)
    got = fread(jpgBuffer, 1, (size_t)len, f);
  fclose(f);
  return (got == (size_t)len) ? (size_t)len : 0;
}
...
void preloadPage(int page)
{
  ...
  size_t fileSize = loadPageFile(path);
  bool success = (fileSize > 0) &&
                 drawJpgWithJpegDec(nextPageSprite, jpgBuffer, fileSize, 0, 0,
                                    DISPLAY_W, DISPLAY_H);
      ...
      else
      {
        for (int i = menuScroll; i < end; i++)
        {
          ...
          else
          {
            ...
            if (xOffset < 0)
              xOffset = 0;
            if (!draw_jpg_file(menuCacheSprite, coverPath, x + 2 + xOffset,
...
void systemShutdown()
{
  ...
  {
    std::string path =
        std::string(PIC_ROOT) + "/" + pics[esp_random() % pics.size()];
    prepareSprite(gSprite, DISPLAY_W, DISPLAY_H, 16, true);
    if (gSprite.getBuffer())
    {
      gSprite.fillScreen(TFT_WHITE);
      if (draw_jpg_file(gSprite, path, 0, 0))
      {
        M5.Display.startWrite();
        gSprite.pushSprite(0, 0);
        M5.Display.display();
...
void drawPage()
{
  ...
  else
  {
    ...
    bool decodeSuccess =
        drawJpgWithJpegDec(gSprite, jpgBuffer, fileSize, 0, 0, DISPLAY_W,
                           DISPLAY_H);
  ...
  int64_t t_preloaded = now_ms();

  // Wait for the e-ink refresh to finish so the timing log below reflects
  // the physical panel update (also prevents piling up refresh requests).
  M5.Display.waitDisplay();
  int64_t t_done = now_ms();
  ESP_LOGI(TAG,
           "page %d: render=%lld push+queue=%lld preload=%lld refresh=%lld ms",
           currentPage + 1, (long long)(t_rendered - t_start),
           (long long)(t_queued - t_rendered),
           (long long)(t_preloaded - t_queued),
           (long long)(t_done - t_preloaded));
